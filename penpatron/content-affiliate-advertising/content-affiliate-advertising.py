...
def get_Content_Affliate_Advertising(content, index):
	...
	#	pass
        ...
        #try:
        #        keywords += map(lambda x: x['text'], get_Text_Concepts(content))
        #except:
        #        pass

        try:
                keywords += map(lambda x: x['text'], get_nltk_ne(content))
        except:
                pass

        try:
                keywords += map(lambda x: x['text'], get_python_tagger(content))
        except:
                pass
	...
	products = getAmazonProducts(keywords, index)
	return products

def getAmazonProducts2(keywords, index):
	...
	for kw in keywords:
		try:
		        response = amazon.ItemSearch(Keywords=kw, SearchIndex=index, ResponseGroup="ItemAttributes")
