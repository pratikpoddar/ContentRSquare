...
def run_twitter_newspaper(sector, tuser, tlist):
	...
	logger.exception('run_twitter_newspaper: ' + sector + ' ' + tuser + ' ' + tlist + ' articleinfo ' + str(ArticleInfo.objects.all().count()) + ' articlesemantics ' + str(ArticleSemantics.objects.all().count()) + ' tweetlinks ' + str(TweetLinks.objects.all().distinct('url').count()))
	return
